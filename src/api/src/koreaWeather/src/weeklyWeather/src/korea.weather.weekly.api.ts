...
export const weeklyWeather = async (): Promise<ResultWeeklyDataProps> => {
  ...
  await axios
    .get(
      `http://apis.data.go.kr/1360000/MidFcstInfoService/getMidTa?serviceKey=${KOREA_WEATHER_API_KEY}&numOfRows=10&pageNo=&dataType=json&regId=11B00000&tmFc=${AFTER3}`
    )
    .then((res) => {
